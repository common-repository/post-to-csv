...
if ( ! class_exists( 'Psttcsv_Settings_Tabs' ) ) {
	/**
	 * Class Psttcsv_Settings_Tabs for Settings
	 */
	class Psttcsv_Settings_Tabs extends Bws_Settings_Tabs {
		/**
		 * All post types from db.
		 *
		 * @var array
		 */
		...
		public function save_options() {
			...
			if ( ! isset( $_POST['psttcsv_field'] )
				...
				print esc_html__( 'Sorry, your nonce did not verify.', 'post-to-csv' );
				exit;
			} else {
				...
				<tr>
					...
					<td>
						<div class="bws_help_warning_wrapper">
							<h4><?php esc_html_e( 'Warning. Read before export', 'post-to-csv' ); ?></h4>
							<div class="bws_help_box dashicons dashicons-editor-help">
								<div class="bws_hidden_help_text">
									<p><?php esc_html_e( 'This CSV file may contain content that could auto-execute formulas in certain spreadsheet applications. This can be problematic as auto-executing formulas can potentially be used for malicious activity, including malware and other security threats.', 'post-to-csv' ); ?></p>
									<p><?php esc_html_e( 'Here are some elements that could be causing these issues:', 'post-to-csv' ); ?></p>
									<ul>
										<li><b><?php esc_html_e( 'Formulas', 'post-to-csv' ); ?></b>: <?php esc_html_e( 'The most direct issue. CSV files can include formulas that will automatically execute when the file is opened in a spreadsheet application. This can be particularly hazardous if the formula includes functions that can interact with the user\'s system, such as creating new files, executing commands on the user\'s computer, or even sending data to a remote server.', 'post-to-csv' ); ?></li>
										<li><b><?php esc_html_e( 'Hyperlinks', 'post-to-csv' ); ?></b>: <?php esc_html_e( 'Hyperlinks may also automatically open upon CSV file launch. If a URL leads to a malicious site, this can pose problems.', 'post-to-csv' ); ?></li>
										<li><b><?php esc_html_e( 'Embedded or OLE Objects', 'post-to-csv' ); ?></b>: <?php esc_html_e( 'Some programs allow the embedding of objects (such as images or other files) within a spreadsheet. If these objects contain malware, they can be activated upon opening the file.', 'post-to-csv' ); ?></li>
										<li><b><?php esc_html_e( 'Macros', 'post-to-csv' ); ?></b>: <?php esc_html_e( 'While CSV files typically do not support macros, some spreadsheet applications may automatically convert them into a format that supports macros, which can also pose a security risk.', 'post-to-csv' ); ?></li>
									</ul>
									<p><?php esc_html_e( 'Always exercise caution when opening files from untrusted sources, especially those that can contain auto-executing scripts or other potentially dangerous content. If possible, disable automatic execution of formulas and other functions in your spreadsheet applications.' ); ?></p>
								</div>
							</div>
						</div>
				...
				<tr>
					...
					<td>
						<div class="bws_help_warning_wrapper">
								...
								<div class="bws_help_box dashicons dashicons-editor-help">
									<div class="bws_hidden_help_text">
										<p><?php esc_html_e( 'This CSV file may contain content that could auto-execute formulas in certain spreadsheet applications. This can be problematic as auto-executing formulas can potentially be used for malicious activity.', 'post-to-csv' ); ?></p>
										<p><?php esc_html_e( 'Here are some elements that could be causing these issues:', 'post-to-csv' ); ?></p>
										<ul>
											<li><b><?php esc_html_e( 'Formulas', 'post-to-csv' ); ?></b>: <?php esc_html_e( 'The most direct issue. CSV files can include formulas that will automatically execute when the file is opened in a spreadsheet application.', 'post-to-csv' ); ?></li>
											<li><b><?php esc_html_e( 'Hyperlinks', 'post-to-csv' ); ?></b>: <?php esc_html_e( 'Hyperlinks may also automatically open upon CSV file launch.', 'post-to-csv' ); ?></li>
											<li><b><?php esc_html_e( 'Embedded or OLE Objects', 'post-to-csv' ); ?></b>: <?php esc_html_e( 'Some programs allow the embedding of objects (such as images or other files) within a spreadsheet. If these objects contain malware, they can be activated upon opening the file.', 'post-to-csv' ); ?></li>
											<li><b><?php esc_html_e( 'Macros', 'post-to-csv' ); ?></b>: <?php esc_html_e( 'While CSV files typically do not support macros, some spreadsheet applications may automatically convert them into a format that supports macros.', 'post-to-csv' ); ?></li>
										</ul>
										<p><?php esc_html_e( 'Always exercise caution when opening files from untrusted sources, especially those that can contain auto-executing scripts or other potentially dangerous content. If possible, disable automatic execution of formulas and other functions in your spreadsheet applications.' ); ?></p>
									</div>
								</div>
		 ...
		 *
		 * @access public
		 * @param bool $save_results Flag for save results.
		 */
		public function display_custom_messages( $save_results ) {
			if ( ! empty( $_SESSION['psttcsv_error_message'] ) ) {
				if ( 'no_data' === $_SESSION['psttcsv_error_message'] ) {
					...
					<?php
				} elseif ( 'failed_put_content' === $_SESSION['psttcsv_error_message'] ) {
					?>
					<div class="error inline psttcsv_error"><p><strong><?php esc_html_e( 'An error occurred while writing data to the file.', 'post-to-csv' ); ?></strong></p></div>
					<?php
				} elseif ( 'file_error' === $_SESSION['psttcsv_error_message'] ) {
					?>
					<div class="error inline psttcsv_error"><p><strong><?php esc_html_e( 'An error occurred while creating the file to record data.', 'post-to-csv' ); ?></strong></p></div>
					<?php
				}
