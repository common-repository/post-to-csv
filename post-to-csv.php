...
if ( ! function_exists( 'psttcsv_plugin_admin_init' ) ) {
	...
	function psttcsv_plugin_admin_init() {
		...
		if ( isset( $_GET['page'] ) && 'post-to-csv.php' === $_GET['page'] ) {
			...
			if ( isset( $_POST['psttcsv_field'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['psttcsv_field'] ) ), 'psttcsv_action' ) && ( $is_general_submit || $is_comments_submit ) ) {
				psttcsv_print_csv();
			}
		}
...
if ( ! function_exists( 'psttcsv_print_csv' ) ) {
	...
	function psttcsv_print_csv() {
		global $wpdb, $psttcsv_options, $wp_filesystem;
		WP_Filesystem();
		$upload_dir = wp_upload_dir();
		$file_name = wp_tempnam( 'tmp', $upload_dir['path'] . '/' );
		if ( ! $file_name ) {
			$_SESSION['psttcsv_error_message'] = 'file_error';
		}
		$export_str = '';

		if ( isset( $_POST['psttcsv_export_submit_comments'] ) && isset( $_POST['psttcsv_field'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['psttcsv_field'] ) ), 'psttcsv_action' ) ) {
			...
			sort( $col_array );
			$export_str .= '"' . implode( '";"', $col_array ) . '";' . PHP_EOL;
			...
			while ( ! empty( $results ) ) {
				foreach ( $results as $result ) {
					...
					ksort( $result );
					$export_str .= '"' . implode( '";"', $result ) . '";' . PHP_EOL;
				}
				$start++;
			...
			if ( ! $result ) {
				$_SESSION['psttcsv_error_message'] = 'failed_put_content';
				return;
			}
